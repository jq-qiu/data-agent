...
def test_current_status_and_resume_instructions_are_consistent() -> None:
    ...
    assert "SQL-018 Post-SQL-017 Real-model Rerun" in _markdown_section(
        status, "## Current Feature"
    )
    assert "SQL-018 Post-SQL-017 Real-model Rerun" in _markdown_section(
        status, "## Last Completed Feature"
    )
    assert "SQL-019 Group-by Join-key Canonicalization" in _markdown_section(
        status, "## Next Feature"
    )
    assert "SQL-019 Spec" in _markdown_section(status, "## Resume From")
