...
| SQL-012 | 限制必需指标列修复引入派生表导致 Trace 丢失 | 已完成；安全单表形状扁平化，未真实复测 |
| SQL-013 | 约束修复 SQL 的分组与 Calendar Join 符合 Plan | 已完成；结构化错误导向约束，未真实复测 |
...
| SQL-024 | Daily GMV DWS Source and Projection Contract | 已完成；T05 Plan 锁地区 DWS 单表与 date_id/SUM(gmv) 投影，Validator AST 校验；未真实复测 |
| SQL-025 | Post-SQL-024 T05 Real-model Rerun | 已完成；27/30，T04/T05/N02 转通过，N01 漂移失败；Gate true |
| SQL-026 | Post-SQL-025 继续修复剩余失败 | 下一项；待用户指定目标后冻结 |
